...
export async function generateMetadata({
  ...
  const articulos = await getArticulos({ publicado: true, categoriaSlug: slug })

  return {
    title: `${categoria.nombre} | Categorías`,
    description: `Explora ${articulos.length} ${articulos.length === 1 ? "artículo" : "artículos"} en la categoría ${categoria.nombre}.`,
    openGraph: {
      title: `${categoria.nombre} | Categorías`,
      description: `Explora ${articulos.length} ${articulos.length === 1 ? "artículo" : "artículos"} en la categoría ${categoria.nombre}.`,
      type: "website",
